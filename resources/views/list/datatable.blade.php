				...
				<tr @isset($data->id) data-id="{{ $data->id }}" @endisset>
					...
					@foreach($dataType->browseRows as $row)
						<td>
							...
							@if($row->type == 'image')
								<img
										src="@if( !filter_var($data->{$row->field}, FILTER_VALIDATE_URL)){{ Admin::image( $data->{$row->field} ) }}@else{{ $data->{$row->field} }}@endif"
										style="width:100px">
							...
							@elseif($row->type == 'checkbox')
								@if($options && property_exists($options, 'list_checkbox') && $options->list_checkbox)
									{!! app('admin')->formField($row, $dataType, $data) !!}
							...
							@elseif($row->type == 'text')
								...
								<div
										class="readmore">{{ mb_strlen( $data->{$row->field} ) > 200 ? substr($data->{$row->field}, 0, 200) . ' ...' : $data->{$row->field} }}</div>
							...
							@elseif($row->type == 'rich_text_box')
								@include('admin::multilingual.input-hidden-bread-browse')
								<div
										class="readmore">{{ mb_strlen( strip_tags($data->{$row->field}, '<b><i><u>') ) > 200 ? substr(strip_tags($data->{$row->field}, '<b><i><u>'), 0, 200) . ' ...' : strip_tags($data->{$row->field}, '<b><i><u>') }}</div>
								...
								@can(isset($serviceButton->apply_permission) ? $serviceButton->apply_permission : 'browse', $data)
									@if(isset($serviceButton->attribute) && isset($data->{$serviceButton->attribute}) && $data->{$serviceButton->attribute})
										<a data-id="{{ $data->{$data->getKeyName()} }}"
											...
											@if(config('admin.views.browse.display_text_on_service_buttons'))
												<span
														class="hidden-xs hidden-sm">{{ isset($serviceButton->title) ? $serviceButton->title : '' }}</span>
											@endif
										</a>
									@endif
								@endcan
			...
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title"><i
								class="admin-trash"></i> {{ __('admin.generic.delete_question') }} {{ strtolower($dataType->display_name_singular) }}
						?</h4>
				</div>
		...
		$(document).ready(function () {
			...
			$('.toggleswitch').change(function () {
				...
				$.post('{{ route('admin.api.update') }}', {
					...
					where: ["id", $(this).closest('tr').data('id')],
					data: data,
					dataType: 'json',
					_token: '{{ csrf_token() }}'
				}).done(function () {
